package org.biswajit.spring.restclient;
...
import org.biswajit.data.entities.AuthTokenInfo;
...
public class SpringRestClient {
    ...
    public static final String REST_SERVICE_URI = "http://localhost:8091/RestAPI/";
     
    public static final String AUTH_SERVER_URI = "http://localhost:8091/RestAPI/oauth/token";
     
    public static final String QPM_PASSWORD_GRANT = "?grant_type=password&username=bbastia&password=password";
    ...
    private static void listAllUsers(AuthTokenInfo tokenInfo){
        ...
        ResponseEntity<List> response = restTemplate.exchange(REST_SERVICE_URI+"/user/list/"+QPM_ACCESS_TOKEN+tokenInfo.getAccess_token(),
                HttpMethod.GET, request, List.class);
        List<LinkedHashMap<String, Object>> usersMap = (List<LinkedHashMap<String, Object>>)response.getBody();
         
        if(usersMap!=null){
            for(LinkedHashMap<String, Object> map : usersMap){
                System.out.println("User : id="+map.get("userId")+", FirstName="+map.get("firstName")+", LastName="+map.get("lastName")+", Address "+map.get("address"));
            }
        }else{
            System.out.println("No user exist----------");
        }
    ...
    private static void createUser(AuthTokenInfo tokenInfo) {
        ...
        RestTemplate restTemplate = new RestTemplate();
        User user = new User("Sarah","paul");
        user.setAddress(null);
        HttpEntity<Object> request = new HttpEntity<Object>(user, getHeaders());
        URI uri = restTemplate.postForLocation(REST_SERVICE_URI+"/user/add/"+QPM_ACCESS_TOKEN+tokenInfo.getAccess_token(),
                request, User.class);
    ...
    private static void updateUser(AuthTokenInfo tokenInfo) {
        ...
        RestTemplate restTemplate = new RestTemplate();
        User user  = new User("biswajit","bastia");
        HttpEntity<Object> request = new HttpEntity<Object>(user, getHeaders());
        ResponseEntity<User> response = restTemplate.exchange(REST_SERVICE_URI+"/user/update/1"+QPM_ACCESS_TOKEN+tokenInfo.getAccess_token(),
